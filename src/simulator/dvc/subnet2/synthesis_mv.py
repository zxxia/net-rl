...
class Synthesis_mv_net(nn.Module):
    ...
    def __init__(self):
        ...
        self.deconv5_new3 = nn.Conv2d(out_channel_mv_new, out_channel_mv_new, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.deconv5_new3.weight.data, math.sqrt(2 * 1))
        torch.nn.init.constant_(self.deconv5_new3.bias.data, 0.01)
        self.relu5_new3 = nn.LeakyReLU(negative_slope=0.1)
        self.deconv6_new3 = nn.Conv2d(out_channel_mv_new, out_channel_mv_new, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.deconv6_new3.weight.data, math.sqrt(2 * 1))
        torch.nn.init.constant_(self.deconv6_new3.bias.data, 0.01)
        self.relu6_new3 = nn.LeakyReLU(negative_slope=0.1)
        self.deconv7_new3 = nn.Conv2d(out_channel_mv_new, out_channel_mv_new, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.deconv7_new3.weight.data, math.sqrt(2 * 1))
        torch.nn.init.constant_(self.deconv7_new3.bias.data, 0.01)
        self.relu7_new3 = nn.LeakyReLU(negative_slope=0.1)
        self.deconv8_new3 = nn.Conv2d(out_channel_mv_new, 2, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.deconv8_new3.weight.data, (math.sqrt(2 * 1 * (out_channel_mv_new + 2) / (out_channel_mv_new + out_channel_mv_new))))
        torch.nn.init.constant_(self.deconv8_new3.bias.data, 0.01)

    def forward(self, x, mv_scale = 1, use_new = False):
        # h = x.dtype != self.deconv1.weight.dtype
        # if h:
        #     x = x.float()
        ...
        if not use_new:
            ...
            x = self.deconv8(x)
            if mv_scale != 1:
                # print("upscaling", mv_scale)
                x = F.interpolate(x, scale_factor=1/mv_scale, mode = "bicubic")
        else:
            # print("mvdec newwww")
            x = self.relu5_new3(self.deconv5_new3(x))
            x = self.relu6_new3(self.deconv6_new3(x))
            x = self.relu7_new3(self.deconv7_new3(x))
            x = self.deconv8_new3(x)     
