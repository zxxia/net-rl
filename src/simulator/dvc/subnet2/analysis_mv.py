...
class Analysis_mv_net(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1_new3 = nn.Conv2d(2, out_channel_mv_new, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.conv1_new3.weight.data, (math.sqrt(2 * (2 + out_channel_mv_new) / (4))))
        torch.nn.init.constant_(self.conv1_new3.bias.data, 0.01)
        self.relu1_new3 = nn.LeakyReLU(negative_slope=0.1)
        self.conv2_new3 = nn.Conv2d(out_channel_mv_new, out_channel_mv_new, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.conv2_new3.weight.data, math.sqrt(2))
        torch.nn.init.constant_(self.conv2_new3.bias.data, 0.01)
        self.relu2_new3 = nn.LeakyReLU(negative_slope=0.1)
        self.conv3_new3 = nn.Conv2d(out_channel_mv_new, out_channel_mv_new, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.conv3_new3.weight.data, math.sqrt(2))
        torch.nn.init.constant_(self.conv3_new3.bias.data, 0.01)

    def forward(self, x, mv_scale = 1, use_new = False):
        if mv_scale != 1:
            # print("downscaling", mv_scale)
            # assert False
            x = F.interpolate(x, scale_factor=mv_scale, mode = "bicubic")
        if not use_new:
            ...
            # print("mvenc newwww")
            x = self.relu1_new3(self.conv1_new3(x))
            x = self.relu2_new3(self.conv2_new3(x))
            return self.conv3_new3(x)
