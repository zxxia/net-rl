...
def plot_fbra_log(log_path, ax0, ax1, ax2):
    df_fbra = pd.read_csv(log_path)

    ax0.plot(df_fbra['timestamp_us']/1e6, df_fbra['p40_owd_ms'], label='P40 owd')
    ax0.plot(df_fbra['timestamp_us']/1e6, df_fbra['p80_owd_ms'], label='P80 owd')

    ax1.plot(df_fbra['timestamp_us']/1e6, df_fbra['state'], 'o-', ms=3)
    ax1.set_yticks([0, 1, 2, 3])
    ax1.set_yticklabels(['Down', 'Stay', "Up", 'Probe'])
    ax1.set_ylabel('FBRA State')
    ax1.set_ylim(0, )
    ax1.set_xlim(0, )
    ax1.legend()

    ax2.plot(df_fbra['timestamp_us']/1e6, df_fbra['corr_owd_low'], 'o-', ms=3, label='corr_owd_low')
    ax2.plot(df_fbra['timestamp_us']/1e6, df_fbra['corr_owd_high'], 'o-', ms=3, label='corr_owd_high')
    ax2.set_xlabel('Time (s)')
    ax2.set_ylabel('Correlated OWD')
    ax2.set_ylim(0, )
    ax2.set_xlim(0, )
    ax2.legend()
...
if os.path.exists(os.path.join(save_dir, 'fbra_log.csv')):
    plot_fbra_log(os.path.join(save_dir, 'fbra_log.csv'), axes[1], axes[6], axes[7])
