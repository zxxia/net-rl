...
from simulator_new.cc.on_rl import on_rl_environment
        ...
        if self.n_calls % self.check_freq == 0:
            if COMM_WORLD.Get_rank() == 0 and self.val_log_writer is not None:
                ...
                for idx, val_trace in enumerate(self.val_traces):
                    ...
                    # avg_tr_min_rtt.append(val_trace.min_delay * 2)
                    df = pd.read_csv(os.path.join(val_sim_dir, 'on_rl_log.csv'))
                    avg_rewards.append(df['reward'].mean())
                    avg_losses.append(df['loss_fraction'].mean())
                    avg_tputs.append(df['tput_Bps'].mean())
                    avg_delays.append(df['owd_ms'].mean())
...
def train_aurora(train_scheduler: TraceScheduler, config_file: str,
    ...
    env = gym.make('OnRLEnv-v1', trace_scheduler=train_scheduler,
                   app=app, lookup_table_path=lookup_table_path, ae_guided=ae_guided)
    env.seed(seed)
    model = PPO1(FeedForwardPolicy, env, verbose=1, seed=seed,
                 ...
                 policy_kwargs={"layers": [64, 32], "feature_extraction":
                                "mlp"})
